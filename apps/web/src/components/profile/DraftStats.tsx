      ...
      <CardContent className="space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          <div className="rounded-lg border border-border bg-ika-700/40 p-4">
            ...
            <div className="mt-2 text-2xl font-semibold text-ink-900">{banFrequency}</div>
          ...
          <div className="rounded-lg border border-border bg-ika-700/40 p-4">
            ...
            <div className="mt-2 text-2xl font-semibold text-ink-900">{pickFrequency}</div>
          ...
          <div className="rounded-lg border border-border bg-ika-700/40 p-4">
            <div className="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-ink-500">
              Draft winrate
              <Badge className="border border-cool-400/30 bg-cool-500/10 text-cool-300">{deltaLabel}</Badge>
            </div>
            <div className="mt-2 text-2xl font-semibold text-ink-900">{draftWinrate}%</div>
            <div className="mt-1 text-xs text-ink-500">Match winrate {matchWinrate}%</div>
            <div className="mt-3 text-xs text-ink-500">Pick success after draft win: {pickSuccess}</div>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-lg border border-border bg-ika-700/40 p-4">
            ...
            <div className="mt-3 h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={bansAgainst} layout="vertical" margin={{ top: 8, right: 16, left: 16, bottom: 8 }}>
                  <XAxis type="number" tick={{ fill: "#8c96a8", fontSize: 11 }} axisLine={false} tickLine={false} />
                  <YAxis
                    type="category"
                    dataKey="name"
        ...
        <div className="rounded-lg border border-border bg-ika-700/40 p-4">
          ...
          <ul className="mt-3 grid gap-2 text-sm text-ink-900 sm:grid-cols-2">
            {sequences.map((item) => (
              <li key={item.sequence} className="flex items-center justify-between rounded-md border border-border bg-ika-800/60 px-3 py-2">
